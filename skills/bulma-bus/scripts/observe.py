#!/usr/bin/env python3
"""Append one review observation to $BULMA_HOME/insights/observations.jsonl.
...
field, derives caught_by_ours from the bulma-review marker's open= list
...
FD_REVIEWERS = ("bulma-fd-reviewer", "bulma-fd-quality")
...
def main() -> int:
    ...
    ap.add_argument("--ours-open", default=None,
                    help="open= value of the latest bulma-review marker on the PR; omit when none exists")
    ap.add_argument("--file", default=None, help="override target (tests)")
    args = ap.parse_args()

    row = build(args)
    target = Path(args.file) if args.file else bulma_home() / "insights" / "observations.jsonl"
