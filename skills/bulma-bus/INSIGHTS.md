...
`$BULMA_HOME/insights/observations.jsonl`. Global, one file for every repo,
...
Writers: `bulma-lstm` `verify` and `bulma-reviewer` `code_review` (human
comments), and `bulma-feature-delivery` `reviewer` and `quality` (our own
...
- the comment, or the review it belongs to, carries a `<!-- bulma-review:`
...
```bash
python3 ../bulma-bus/scripts/observe.py \
  ...
  --ours-open "<open= value of the latest bulma-review marker>"
```

Resolve `../bulma-bus/` from the calling skill's directory. The script
...
| Field | Rule |
|---|---|
| `--axis` | The `bulma-code-review` axis the comment would have been filed under. `other` only when none fits |
...
| `--ours-open` | Copy the `open=` value from the newest `<!-- bulma-review:` marker on the PR. Omit the flag when the PR has no marker. The script turns it into `caught_by_ours`: `yes` if our review had a finding on the same path within 10 lines, `no` if not, `unknown` without a marker |
...
- Write a human-comment row (`lstm`, `reviewer`) for a bot, our own GitHub
  review (the `bulma-review` marker), or the PR author. Our gates write
...
```bash
python3 ../bulma-bus/scripts/observe.py --source fd \
  --reviewer bulma-fd-reviewer|bulma-fd-quality \
