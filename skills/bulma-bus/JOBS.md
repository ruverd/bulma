...
Under `$BULMA_ROOT` ([DISK.md](DISK.md)). **Never** the git root.
...
| `QA_REQUEST` / `/bulma-qa` | other QA `qa_active` set | **enqueue**. Never start a second QA. |
| `QA_REQUEST` / `/bulma-qa` | no `qa_active` | start QA (claim `qa_active`). |
...
2. If main is **idle** for this call → `lane=foreground`.
   Write `.bulma-<kind>/STATE.md`. Continue the graph.
...
3. If main is **busy** → `lane=worker`:
   ...
   - Record `worktree`, `worker_id`. Chat (`bulma-memory`): job id + path.
   - **Do not** leave the graph that already owns main.

Foreground job keeps `.bulma-developer/STATE.md` (or reviewer / lstm).
Worker jobs use `.bulma-bus/jobs/<id>/STATE.md` only.
...
Prefer host `spawn_worker` with worktree isolation ([bulma-host](../bulma-host/SKILL.md)).
...
`git worktree remove` + `git branch -D` by a human after `bulma status`
lists orphans. No auto-remove. `bulma status` also prints the cwd job
...
Child type: general-purpose ([bulma-host](../bulma-host/SKILL.md) `spawn_worker`).
**Never** spawn types `bulma_developer` / `bulma_qa` / `bulma_triage` /
`bulma_reviewer` / `bulma_lstm`.
Worker **must not** spawn children.

Prompt must include: job id, worktree path (or “host worktree”),
PR/ticket, “execute the skill **yourself** — no bulma-* / bulma-fd-*
spawns”, never merge, stay draft.

| kind | Worker does |
|---|---|
| developer | `bulma-feature-delivery` nodes inline (or Fix contract). Draft PR. Write `jobs/<id>/RESULT.md`. |
| reviewer | `bulma-code-review` for **one** PR only. Write `RESULT.md`. |
| lstm | `bulma-lstm` nodes inline for **one** PR. Same branch. TDD via coder contract (spawn `bulma-fd-coder` on foreground; inline on depth-1 worker). Write `RESULT.md`. |

On RESULT: update JOBS. Developer worker with green+MERGEABLE →
[Enqueue or start QA](#enqueue-or-start-qa). CI pending → start
`bulma-goal` loop for **that** PR (one loop per PR).
...
3. Reset `.bulma-qa/STATE.md` from the template. The old STATE describes
   another PR.
...
3. **Taken:** append `<id>` to `qa_waiting`. Do **not** switch.
   Do **not** touch `.bulma-qa/STATE.md`. Chat: queue position.
...
3. If `qa_waiting` is non-empty: dequeue head → `qa_active`,
   ...
   load QA graph. Reset `.bulma-qa/STATE.md` from the template
   for the new PR.

Standalone `/bulma-qa` (no developer on stack): same dequeue
after verdict.
...
- Overwriting `.bulma-qa/STATE.md` for a queued PR
