...
Everything under **`$BULMA_ROOT`** ([DISK.md](../bulma-bus/DISK.md)) — **never**
the git root, a worktree, or `git add`:

```
$BULMA_ROOT/.bulma-feature-delivery/
...
Claude and Grok on this Mac read the same `$BULMA_ROOT`. Do not commit.

## Template `.bulma-feature-delivery/HANDOFF.md`
...
/bulma-fd resume
# or
/bulma-feature-delivery resume from $BULMA_ROOT/.bulma-feature-delivery/HANDOFF.md
```
```
...
2. Confirm `$BULMA_ROOT/.bulma-feature-delivery/HANDOFF.md` + `STATE.md`. If missing:
   **rebuild** from git/gh/tracker (fallback), do not restart from a raw goal.
3. Invoke: **`/skills bulma-feature-delivery`** and ask **resume**
   (`/bulma-fd` **does not exist** as a Grok slash command — those are built-in).
...
Same: checkout branch, `/bulma-fd resume` (real command on Claude), **RECONCILE**,
...
D: wrote $BULMA_ROOT/.bulma-feature-delivery/HANDOFF.md + STATE (global, not git)
P: open the other runtime on branch <feature/<id-lowercase>> and run:
   Claude: /bulma-fd resume · Grok: /skills bulma-feature-delivery + "resume"
