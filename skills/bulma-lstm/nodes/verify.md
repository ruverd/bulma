...
| `risk` | low / normal / elevated. Comment text + bound path. Tokens from [ROUTING.md](../../bulma-feature-delivery/ROUTING.md) §Risk (auth, tenant, billing, …). Do not run `classify-risk.py` here (no file list yet) |
...
`../bulma-bus/scripts/observe.py --source lstm` (path from this skill's
directory), passing the row's `claim_true`.
[INSIGHTS.md](../../bulma-bus/INSIGHTS.md) defines who counts as human, the
