...
Paths live under `$CAPTURE_DIR` or `.bulma-qa/artifacts`.
...
../scripts/publish-evidence.sh \
  ...
  --body-file "$BODY" \
  --video "$VIDEO" --artifacts .bulma-qa/artifacts
```
...
<!-- bulma-qa: v=1 verdict=PASS sha=<sha> -->
```
...
`bulma-qa: v=1 … sha=<sha>` already exists, do not post another.
