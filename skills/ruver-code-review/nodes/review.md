...
- Tracker ticket: extract `[A-Z][A-Z0-9]+-\d+` from `headRefName` or title. If
  ...
  ([ruver-host](../../ruver-host/SKILL.md), Optional MCP). If the capability is
  absent or no ID exists, continue on the PR body alone.

Write `$RUVER_ROOT/.ruver-code-review/AC.md` before any product-file Read:
...
Write the survivors to `$RUVER_ROOT/.ruver-code-review/findings.json`. Then:

```bash
python3 scripts/bind-findings.py \
  --patch "$RUVER_ROOT/.ruver-code-review/PATCH.diff" \
  --findings "$RUVER_ROOT/.ruver-code-review/findings.json" \
