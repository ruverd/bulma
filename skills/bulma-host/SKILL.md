...
After install they are siblings (`bulma-qa` next to `bulma-bus`
and `unslop`).
...
| `load_graph name` | Same as `load_skill bulma-<name>`. Main thread only | Same |
...
`$BULMA_HOME/bulma.json`. What each host can apply per call (checked
2026-09):
...
Never `spawn_worker` with types `bulma_developer` / `bulma_qa` /
`bulma_triage` / `bulma_reviewer` / `bulma_lstm`. Those are
`load_graph` on the main thread.

Fd **workers** (`bulma-fd-coder`, `bulma-fd-tester`, …) may be the
child type when the host registered them. If it did not, use
general-purpose and paste the node file into the prompt.

## schedule_wake

Used by `bulma-goal` and `bulma-code-review` wait_ci. Interval 5m.
...
| `tracker_save_issue` | Linear: `mcp__linear-server__save_issue` | `gh issue create` / `glab issue create` ([BLOCKERS.md](../bulma-feature-delivery/BLOCKERS.md)) |
...
depends on. See
[MCP_CONTEXT.md](../bulma-feature-delivery/MCP_CONTEXT.md).

## Starting graphs

| Host | How the user starts a graph |
|---|---|
| Grok | `/developer` (command alias) or `/bulma-developer` (skill name) |
| Claude Code | `/developer` or `/bulma-developer` |
| Codex | `$bulma-developer` or select `bulma-developer` through `/skills` |
| Cursor | `/bulma-developer` |

Codex reserves slash input for built-in commands. Never tell a Codex user to
run `/bulma-*`; the parser rejects it before skill routing.

Short command aliases (Grok / Claude, via `commands/`): `/developer`,
`/reviewer`, `/lstm`, `/qa`, `/memory`. Same skill. Skill folder names
stay `bulma-*`. `/memory` is `bulma-memory` (lib), not a graph.

## Disk

`$BULMA_ROOT` is host-neutral. See [skills/bulma-bus/DISK.md](../bulma-bus/DISK.md).
Claude, Codex, Grok, and Cursor on one machine **share** that tree.

## Quality rubric

`bulma-fd-quality` loads bundled
...
system: [PRODUCT.md](../bulma-feature-delivery/PRODUCT.md)
plus the **current repo** (`AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING`)
plus `bulma-memory` (confirmed reviewers, chat language).
