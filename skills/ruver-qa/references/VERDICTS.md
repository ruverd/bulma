...
| Verdict | When |
|---|---|
| `PASS` | Surface holds, including walked user-break steps. Every Must AC was observed in the running app or the running API (app-level). Unproven Must is not PASS. A happy-only walk is not PASS. A login-wall / Check-your-email .webm is not PASS (re-record or BLOCKED). API/backend: an attached HTTP still (PNG of status + body) of the changed endpoints, or stills/clips of the FE screens that call them. A .webm is not required when every planned step is `kind: endpoint`. Or triage said `NOT_A_BUG` and nothing else failed. Or triage said `NEW_BUG` / `EXISTING_BUG` (unrelated) and this PR's ACs still hold. |
...
the QA header and the walk video (per-surface reel) and/or HTTP
stills.
