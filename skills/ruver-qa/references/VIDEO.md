...
path. Path: `.ruver-qa/artifacts/S<n>-<slug>.webm`.
...
```
   ...
   `$STILL` is `.ruver-qa/artifacts/S<n>-pass.png`. Primary control
...
```bash
../scripts/concat-clips.sh --out .ruver-qa/artifacts/reel.webm \
  .ruver-qa/artifacts/S*.webm
```
