...
trap 'rm -rf "$TMP"' EXIT
export BULMA_HOME="$TMP/home"
RR="$TMP/root"
mkdir -p "$BULMA_HOME" "$RR"
...
python3 - "$BULMA_HOME/bulma.json" <<'PY'
import json, sys
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage.json" \
  ...
  --bulma-root "$RR" --json >"$J" || fail "ask fd.triage exit"
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage-078.json" \
  --power cautious --bulma-root "$RR" --json >"$J" || fail "ask cautious exit"
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage-078.json" \
  --power bold --bulma-root "$RR" --json >"$J" || fail "ask bold exit"
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage.json" \
  --power shadow --graph-answer path=debug_fix --bulma-root "$RR" --json >"$J" || fail "ask shadow exit"
...
python3 "$BULMA" ask policy.ask --state "$FIX/state-policy-ask.json" --replay "$FIX/replay-policy-ask.json" \
  --bulma-root "$RR" --json >"$J" || fail "ask policy exit"
...
python3 "$BULMA" ask policy.ask --state "$FIX/state-policy-ask.json" --replay "$FIX/replay-policy-ask.json" \
  --power cautious --bulma-root "$RR" --json >"$J" || fail "ask policy cautious exit"
...
# --- dynamic criteria ---
if python3 "$BULMA" ask entry.next_step --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-next-step.json" --bulma-root "$RR" --json >/dev/null 2>&1; then
...
python3 "$BULMA" ask entry.next_step --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-next-step.json" \
  --criteria "$FIX/criteria-next-step.json" --bulma-root "$RR" --json >"$J" || fail "dynamic ask exit"
...
python3 "$BULMA" ask entry.next_step --state "$FIX/state-fd-triage.json" --criteria "$FIX/criteria-next-step.json" --dry-run --bulma-root "$RR" >"$J" || fail "dry-run exit"
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-secret.json" --dry-run --bulma-root "$RR" >"$J" || fail "dry-run secret exit"
...
PY
python3 "$BULMA" ask fd.triage --state "$TMP/big.json" --replay "$FIX/replay-fd-triage.json" --bulma-root "$RR" --json >"$J" || fail "big state exit"
...
TSV="$RR/.bulma-core/DECISIONS.tsv"
need "$TSV"
python3 - "$TSV" <<'PY'
import sys
...
out="$(python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage.json" --bulma-root "$RR" --line)"
...
out="$(python3 "$BULMA" ask policy.ask --state "$FIX/state-policy-ask.json" --replay "$FIX/replay-policy-ask.json" --power shadow --bulma-root "$RR" --line)"
...
PY
out="$(python3 "$BULMA" ask-many --batch "$TMP/batch.json" --bulma-root "$MR")"
...
python3 - "$MR/.bulma-core/DECISIONS.tsv" <<'PY'
import sys
...
PY
python3 "$BULMA" ask-many --batch "$TMP/batch.json" --bulma-root "$MR" --json >"$J" || fail "ask-many --json"
...
PY
before="$(wc -l <"$MR/.bulma-core/DECISIONS.tsv")"
set +e; python3 "$BULMA" ask-many --batch "$TMP/bad.json" --bulma-root "$MR" >/dev/null 2>&1; code=$?; set -e
[[ "$code" -eq 4 ]] || fail "bad batch item must exit 4 before any ask, got $code"
[[ "$(wc -l <"$MR/.bulma-core/DECISIONS.tsv")" == "$before" ]] || fail "bad batch must not log anything"
ok ask-many
...
python3 "$BULMA" dispatch plan --tickets "$DT" --risk normal --path full_feature --host claude \
  --replay "$FIX/replay-dispatch-light.json" --context repo=o/r --context pr=9 --bulma-root "$DR" --json >"$J" || fail "dispatch plan shadow exit"
...
python3 "$BULMA" dispatch plan --tickets "$DT" --ticket 1 --risk normal --host claude --power balanced \
  --replay "$FIX/replay-dispatch-light.json" --bulma-root "$DR" --json >"$J" || fail "dispatch plan unmapped exit"
...
python3 "$BULMA" dispatch plan --tickets "$DT" --risk normal --path full_feature --host claude --power balanced \
  --files src/inbox/Empty.tsx --replay "$FIX/replay-dispatch-light.json" --context pr=9 --bulma-root "$DR" --json >"$J" || fail "dispatch plan live exit"
...
python3 "$BULMA" dispatch plan --tickets "$DT" --ticket 3 --risk elevated --host claude --power balanced \
  --replay "$FIX/replay-dispatch-light.json" --bulma-root "$DR" --json >"$TMP/elev.json" || fail "dispatch plan elevated exit"
...
python3 "$BULMA" dispatch plan --tickets "$DT" --ticket 3 --host claude --power balanced \
  --replay "$FIX/replay-dispatch-unsure.json" --bulma-root "$DR" --json >"$TMP/unsure.json" || fail "dispatch plan unsure exit"
[[ "$(jget "$TMP/unsure.json" 0.tier)" == "heavy" && "$(jget "$TMP/unsure.json" 0.clamp)" == "undecided" ]] || fail ".62 < .80 must run heavy"
if python3 "$BULMA" dispatch plan --tickets "$DT" --ticket 9 --bulma-root "$DR" >/dev/null 2>&1; then fail "unknown ticket id accepted"; fi
...
out="$(python3 "$BULMA" dispatch result "$U1" fail --stage test --tokens 12000 --host claude --bulma-root "$DR")"
grep -F -q 'tier=standard spawn=model=sonnet loops=1' <<<"$out" || fail "fail must escalate light->standard: $out"
out="$(python3 "$BULMA" dispatch result "$U1" fail --stage review --host claude --bulma-root "$DR")"
grep -F -q 'tier=heavy spawn=inherit loops=2' <<<"$out" || fail "second fail must escalate to heavy: $out"
python3 "$BULMA" dispatch result "$U1" pass --tokens 3000 --bulma-root "$DR" >/dev/null || fail "dispatch result pass"
python3 "$BULMA" dispatch reverse --file src/auth/session.ts --pr 9 --source qa --bulma-root "$DR" >"$TMP/rev.out" || fail "dispatch reverse"
grep -F -q 'reversed 2 unit(s) from qa' "$TMP/rev.out" || fail "reverse by file+pr: $(cat "$TMP/rev.out")"
python3 - "$DR/.bulma-core/DISPATCH.tsv" "$DR/.bulma-core/DECISIONS.tsv" "$U1" <<'PY'
import sys
...
PY
out="$(python3 "$BULMA" dispatch report --repo-only --bulma-root "$DR")"
...
out="$(python3 "$BULMA" dispatch review --repo-only --bulma-root "$DR")"
...
GR="$TMP/groot"; mkdir -p "$GR/.bulma-core"
PYTHONDONTWRITEBYTECODE=1 python3 - "$GR/.bulma-core/DISPATCH.tsv" "$SKILL/scripts" <<'PY'
import sys
...
PY
out="$(python3 "$BULMA" dispatch report --repo-only --bulma-root "$GR")"
...
grep -F -q '/developer, /qa, /reviewer, /lstm, /bulma-triage' <<<"$out" || fail "doctor must point at the plain graphs"
set +e
...
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage.json" \
  --power cautious --bulma-root "$RR" --json >"$J" || fail "ask with tune exit"
[[ "$(jget "$J" answers.path.act_at)" == "0.7" ]] || fail "tuned .60 + cautious .10 must be .70, got $(jget "$J" answers.path.act_at)"
python3 "$BULMA" ask fd.triage --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-fd-triage.json" \
  --bulma-root "$RR" --dry-run >"$J" || fail "dry-run model exit"
...
python3 "$BULMA" outcome "$DID" work_kind reversed --note "was a feature" --bulma-root "$RR" >/dev/null || fail "outcome exit"
if python3 "$BULMA" outcome "$DID" nope reversed --bulma-root "$RR" >/dev/null 2>&1; then fail "outcome accepted an unknown question"; fi
python3 - "$TSV" "$DID" <<'PY'
import sys
rows = [l.split("\t") for l in open(sys.argv[1], encoding="utf-8").read().splitlines()[1:]]
...
REP="$TMP/rep"
mkdir -p "$REP/.bulma-core"
python3 - "$REP/.bulma-core/DECISIONS.tsv" <<'PY'
import sys
...
PY
out="$(python3 "$BULMA" report --repo-only --bulma-root "$REP")"
...
need "$WORLD"
W="$TMP/world.json"
PATH="$FIX/bin:$PATH" bash "$WORLD" --bulma-root "$FIX/world" --out "$W" >/dev/null || fail "world.sh exit"
[[ "$(jget "$W" stack_top)" == "qa" ]] || fail "stack_top"
[[ "$(jget "$W" jobs.qa_active)" == "qa-pr-812" ]] || fail "qa_active"
[[ "$(jget "$W" user)" == "octocat" ]] || fail "user login"
...
PATH="$FIX/bin:$PATH" bash "$WORLD" --bulma-root "$FIX/world" --pr https://github.com/o/r/pull/805 --out "$W" >/dev/null || fail "world.sh --pr exit"
[[ "$(jget "$W" pr.author_is_user)" == "true" ]] || fail "pr.author_is_user"
[[ "$(jget "$W" pr.review_decision)" == "CHANGES_REQUESTED" ]] || fail "pr.review_decision"
PATH="$FIX/bin-broken:$(dirname "$(command -v python3)"):/usr/bin:/bin" bash "$WORLD" --bulma-root "$FIX/world" --out "$W" >/dev/null || fail "world.sh without gh must still exit 0"
...
[[ ! -e "$FIX/world/.bulma-core/world.json" ]] || fail "world.sh wrote into the fixture root despite --out"
ok world
...
mkstate() {  # workspace graph age_hours body
  mkdir -p "$WH/$1/.bulma-$2"
  printf '%s\n' "$4" >"$WH/$1/.bulma-$2/STATE.md"
  touch -t "$(python3 -c 'import sys,time; print(time.strftime("%Y%m%d%H%M", time.localtime(time.time()-float(sys.argv[1])*3600)))' "$3")" "$WH/$1/.bulma-$2/STATE.md"
}
...
WL="$TMP/wl-home/slug"
mkdir -p "$WL/.bulma-reviewer" "$WL/.bulma-lstm" "$WL/.bulma-developer"
printf '# Reviewer STATE\n\nstatus: escalated\npr_url: https://github.com/o/r/pull/9\njob_id: rev-pr-9\n\n## Notes\nstatus: waiting_user\n' >"$WL/.bulma-reviewer/STATE.md"
printf '# LSTM STATE\n\nstatus: done\njob_id: lstm-pr-8\n' >"$WL/.bulma-lstm/STATE.md"
printf -- '---\nstatus: waiting_user\npr_url: https://github.com/o/r/pull/7\njob_id: dev-7\n---\n' >"$WL/.bulma-developer/STATE.md"
printf '{"closed": {"https://github.com/o/r/pull/7": "MERGED"}}' >"$TMP/wl-home/bulma-watch.json"
W2="$TMP/wl.json"
PATH="$FIX/bin-broken:$(dirname "$(command -v python3)"):/usr/bin:/bin" bash "$WORLD" --bulma-root "$WL" --out "$W2" >/dev/null || fail "world.sh legacy exit"
...
assert row["guard"] == "bulma-code-review Phase 5", row
PY
...
for i in range(3):
    rows.append({"id": "fd-dev-%d-sibling" % i, "source": "fd", "reviewer": "bulma-fd-reviewer", "job": "dev-%d" % i,
...
SR="$TMP/sroot"
PATH="$FIX/bin:$PATH" bash "$WORLD" --bulma-root "$FIX/world" --out "$W" >/dev/null || fail "world.sh for builders"
out="$(python3 "$BULMA" state entry.route --args "review https://github.com/o/r/pull/805" --world "$W" --bulma-root "$SR")"
[[ -f "$out" && "$out" == */sroot/.bulma-core/state/entry.route-*.json ]] || fail "state path: $out"
[[ "$(jget "$out" world.stack_top)" == "qa" ]] || fail "entry.route world.stack_top"
[[ "$(jget "$out" user_login)" == "octocat" ]] || fail "entry.route user_login"
...
paths="$(python3 "$BULMA" state entry.next_step --resume --world "$W" --bulma-root "$SR")"
...
out="$(python3 "$BULMA" state review.risk --pr-json "$FIX/pr-805.json" --bulma-root "$SR")"
[[ "$(jget "$out" churn)" == "140" ]] || fail "review.risk churn"
[[ "$(jget "$out" files.1)" == "api/inbox.py" ]] || fail "review.risk files"
out="$(python3 "$BULMA" state reviewer.failure_class --pr-json "$FIX/pr-805.json" --check-name test --log-file "$FIX/ci-fail.log" --bulma-root "$SR")"
...
if python3 "$BULMA" state qa.gate --bulma-root "$SR" >/dev/null 2>&1; then fail "qa.gate has no builder and must exit 4"; fi
python3 "$BULMA" ask review.risk --build --pr-json "$FIX/pr-805.json" --bulma-root "$SR" --dry-run >"$J" || fail "ask --build dry-run"
grep -F -q '"churn": 140' "$J" || fail "ask --build sends the built state"
if python3 "$BULMA" ask review.risk --build --state "$FIX/state-fd-triage.json" --bulma-root "$SR" --dry-run >/dev/null 2>&1; then fail "--build with --state must exit 4"; fi
ok state-builders
...
grep -F -q 'description: Jev-gated router and overlay for bulma graphs. Use when /bulma.' "$SKILL/SKILL.md" || fail "SKILL.md description must match the budgeted text"
...
python3 "$BULMA" ask entry.next_step --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-next-step-095.json" \
  --criteria "$FIX/criteria-next-step.json" --power cautious --bulma-root "$RR" --json >"$J" || fail "ask entry cautious exit"
...
python3 "$BULMA" ask entry.next_step --state "$FIX/state-fd-triage.json" --replay "$FIX/replay-next-step-095.json" \
  --criteria "$FIX/criteria-next-step.json" --power shadow --bulma-root "$RR" --json >"$J" || fail "ask entry shadow exit"
