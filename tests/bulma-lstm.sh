...
OBSERVE="$ROOT/skills/bulma-bus/scripts/observe.py"
need "$OBSERVE"
need "$ROOT/skills/bulma-bus/INSIGHTS.md"
has 'observe.py --source lstm' "$LSTM/nodes/verify.md"
has 'observe.py --source reviewer' "$ROOT/skills/bulma-reviewer/nodes/code_review.md"
...
out="$(fd --reviewer bulma-fd-reviewer --job dev-abc-1 --finding-id spy-in-try)"
[[ "$out" == "written fd-dev-abc-1-spy-in-try caught_by_ours=self" ]] || fail "observe fd: $out"
out="$(fd --reviewer bulma-fd-quality --job dev-abc-1 --finding-id spy-in-try)"
...
rc=0; fd --reviewer bulma-fd-reviewer --job dev-abc-1 --finding-id "Not A Slug" 2>/dev/null || rc=$?
...
has 'observe.py --source fd --reviewer bulma-fd-reviewer' "$ROOT/skills/bulma-feature-delivery/nodes/reviewer.md"
has 'observe.py --source fd --reviewer bulma-fd-quality' "$ROOT/skills/bulma-feature-delivery/nodes/quality.md"
ok observations-fd
